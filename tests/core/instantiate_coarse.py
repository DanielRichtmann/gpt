...
for clvl in range(num_coarse_levels):
    mglvl = clvl + 1
    cop.append(g.qcd.fermion.coarse_operator(grid, {
        'hermitian': 1,
        'level': clvl,
        'nbasis': nbasis,
    }))
    ...
    cvec_out[clvl][:] = 0
    print("mglvl = %d, clvl = %d: Finished setup" % (mglvl, clvl))

# apply coarse operator
for clvl in range(num_coarse_levels):
    mglvl = clvl + 1
    cop[clvl].M(cvec_in[clvl], cvec_out[clvl])
    print("mglvl = %d, clvl = %d: in = %e" % (mglvl, clvl, g.norm2(cvec_in[clvl])))
    print("mglvl = %d, clvl = %d: out = %e" % (mglvl, clvl, g.norm2(cvec_out[clvl])))

...
template<typename T>
void cgpt_block_masked_inner_product(cgpt_Lattice_base* _coarse, typename Lattice<T>::tensor_reduced& fineMask, Lattice<T>& fineX, Lattice<T>& fineY) {

  typedef typename Lattice<T>::vector_type vCoeff_t;

  blockMaskedInnerProduct(compatible< iSinglet<vCoeff_t> >(_coarse)->l, fineMask, fineX, fineY);
}
